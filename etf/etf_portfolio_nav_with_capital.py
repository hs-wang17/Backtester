...
class ETFPortfolioCalculator:
    def __init__(
        self, strategy_file_path, nav_data_path, commission_rate=0.0, init_capital=1e8, output_dir="/home/haris/project/etf/output", weight_method="equal"
    ):
        """
        初始化ETF组合计算器
        ...
        output_dir: 输出目录，默认为"/home/haris/project/etf/output"
        weight_method: 权重分配方法，可选"equal"(等权重)、"s1"、"s2"，默认为"equal"
        """
    ...
    def load_strategy_data(self):
        ...
        elif self.weight_method in ["s1", "s2"]:
            self.strategy_data = self.strategy_data[["date", "code", self.weight_method]].copy()
        else:
            raise ValueError(f"不支持的权重方法: {self.weight_method}，请选择'equal'、's1'或's2'")
    ...
    def calculate_weights(self, holdings):
        ...
        返回:
        weights_dict: {ETF代码: 权重} 的字典
        """
        if self.weight_method == "equal":
            # 等权重分配
            etf_codes = holdings["code"].tolist()
            n = len(etf_codes)
            return {code: 1.0 / n for code in etf_codes}

        elif self.weight_method in ["s1", "s2"]:
            # 使用s1或s2列作为权重
            weight_col = self.weight_method
            etf_weights = {}
            total_weight = holdings[weight_col].sum()
            ...
            for _, row in holdings.iterrows():
                etf_weights[row["code"]] = row[weight_col] / total_weight
    ...
    def calculate_portfolio_nav(self):
        ...
        first_weights = self.calculate_weights(first_holdings)

        # 计算每只ETF的初始投入资金和份额
        for etf_code, weight in first_weights.items():
        ...
        # 记录前一日的ETF组合
        prev_etf_codes = set(first_weights.keys())

        # 创建策略日期到ETF组合的映射
        strategy_etf_map = {}
        for date in strategy_dates:
            holdings = self.strategy_data[self.strategy_data["date"] == date]
            weights = self.calculate_weights(holdings)
            strategy_etf_map[date] = set(weights.keys())
        ...
        for trade_day in all_relevant_trade_days:
            ...
            if prev_etf_codes != current_etf_codes and current_strategy_date is not None:
                ...
                current_holdings = self.strategy_data[self.strategy_data["date"] == current_strategy_date]
                current_weights = self.calculate_weights(current_holdings)

                for etf_code, weight in current_weights.items():
                    etf_capital = total_assets * weight
                    etf_nav_data = day_nav[day_nav["ts_code"] == etf_code]
                    if len(etf_nav_data) > 0:
                        nav_value = etf_nav_data["accum_nav"].iloc[0]
                        if pd.notna(nav_value) and nav_value > 0:
                            self.etf_shares[etf_code] = etf_capital / nav_value
                            self.logger.info(
                                f"{trade_day}交易日 - ETF {etf_code}: 重新投入 {etf_capital:,.0f} 元 (权重:{weight:.4f}), 净值 {nav_value:.4f}, 份额 {self.etf_shares[etf_code]:.0f}"
                            )
            ...
            if valid_etfs > 0:
                ...
                if current_strategy_date is not None:
                    ...
                    if trade_day == strategy_trade_day:
                        ...
                        prev_weights = self.calculate_weights(prev_holdings)
                        prev_codes = set(prev_weights.keys()) if len(prev_holdings) > 0 else set()
    ...
    def run_analysis(self, show_absolute_value=False):
        ...
        # 绘制净值曲线
        plot_path = os.path.join(self.output_dir, f"portfolio_nav_curve_{self.weight_method}.png")
        self.plot_nav_curve(save_path=plot_path, show_absolute_value=show_absolute_value)

        # 保存结果
        results_path = os.path.join(self.output_dir, f"portfolio_nav_results_{self.weight_method}.csv")
...
def main():
    """主函数"""
    # 权重方法选择，可以修改为 "equal", "s1", "s2"
    weight_method = "equal"  # 默认使用等权重

    # 创建计算器实例
    calculator = ETFPortfolioCalculator(
        strategy_file_path="/home/haris/project/etf/data/ETF轮动策略底层3-GF_with_score.xlsx",
        nav_data_path="/home/haris/project/etf/data/daily_nav",
        commission_rate=0.0,  # 手续费率设为0.0
        init_capital=1e8,  # 1亿初始资金
        weight_method=weight_method,  # 权重分配方法
    )
